...
bool allEqual(long x,long y,long z) {
	x=abs(x);
	y=abs(y);
	z=abs(z);
	return (x==y && x==z && x==y);
...
bool allEqual4D(long x,long y,long z,long t) {
	x=abs(x);
	y=abs(y);
	z=abs(z);
        t=abs(t);
	return (x==y && x==z && x==y && x==t);
}

template <class NewType>
int getValueNeighbours(void * New_void,long index,int *dimensions,bool spheric) {
  NewType * result = static_cast<NewType *>(New_void);
  long numvox=dimensions[0]*dimensions[1]*dimensions[2]*dimensions[3]*dimensions[4];
  long pos=0;
  int val=std::numeric_limits<int>::max();
  for(long shiftz=-1;shiftz<=1;shiftz++) {
    for(long shifty=-1;shifty<=1;shifty++) {
      for(long shiftx=-1;shiftx<=1;shiftx++) {
	if(spheric || !allEqual(shiftx,shifty,shiftz)) {
		pos=index+(shiftx+dimensions[0]*(shifty+(dimensions[1]*shiftz)));
		if(pos>=0 && pos<numvox && result[pos]>0 && val>result[pos]) {
		  val=result[pos];
		}
	}
      }
...
template NIFTYSEG_WINEXPORT int getValueNeighbours<unsigned char>(void * New,long index,int *dimensions,bool spheric);
template NIFTYSEG_WINEXPORT int getValueNeighbours<float>(void * New,long index,int *dimensions,bool spheric);


template <class NewType>
void updateNeighbours(void * New_void,long index,int *dimensions,bool spheric,int depth) {
  NewType * result = static_cast<NewType *>(New_void);
  long pos=0;
  if (depth>0) {
  for(signed char shiftt=-1;shiftt<=1;shiftt++) {
    for(signed char shiftz=-1;shiftz<=1;shiftz++) {
      for(signed char shifty=-1;shifty<=1;shifty++) {
        for(signed char shiftx=-1;shiftx<=1;shiftx++) {
	  if(spheric || !allEqual4D(shiftx,shifty,shiftz,shiftt)) {
	 	pos=index+(shiftx+dimensions[0]*(shifty+(dimensions[1]*shiftz)))+shiftt*dimensions[0]*dimensions[1]*dimensions[2];
		if(pos>=0 && 
		   pos<(dimensions[0]*dimensions[1]*dimensions[2]*dimensions[3]*dimensions[4]) && 
		   result[pos]>0 && 
		   result[index]!=result[pos]) {
		  result[pos]=result[index];
		  updateNeighbours<NewType>(static_cast<void*>(result),pos,dimensions,spheric,depth-1);
		}
	  }
        }
      }
...
template NIFTYSEG_WINEXPORT void updateNeighbours<unsigned char>(void * New,long index,int *dimensions,bool spheric,int depth);
template NIFTYSEG_WINEXPORT void updateNeighbours<float>(void * New,long index,int *dimensions,bool spheric,int depth);


template <class OldType, class NewType>
void ConsecutivelyConnectComp(void * Old_void, void * New_void, ImageSize * Currentsize,bool spheric)
{
    OldType * InputImagePtr = static_cast<OldType *>(Old_void);
    NewType * result = static_cast<NewType *>(New_void);
    int dimensions[5];
    dimensions[0]=Currentsize->xsize;
    dimensions[1]=Currentsize->ysize;
    dimensions[2]=Currentsize->zsize;
    dimensions[3]=(Currentsize->usize>1)?Currentsize->usize:1;
    dimensions[4]=(Currentsize->tsize>1)?Currentsize->tsize:1;
	
    long nelem=dimensions[0]*dimensions[1]*dimensions[2]*dimensions[3]*dimensions[4];
    ...
    long val=0;
    for(long ii=0; ii<nelem; ii++) {
        if(InputImagePtr[ii]!=0) {
	    val=getValueNeighbours<NewType>(static_cast<void*>(result),ii,dimensions,spheric);
		...
		result[ii]=val;
		// To avoid segmentation fault with very big connected lesions we need to cut the 
		// the number of recursive calls
		updateNeighbours<NewType>(static_cast<void*>(result),ii,dimensions,spheric,75000);
            }
        }
    }
    int num_roi=1;
    long find=0;
    for(int val_roi=1;val_roi<count;val_roi++) {
	find=0;
	long ii=0;
	for(ii=0; ii<nelem; ii++) {
	    if(val_roi==result[ii]) {
		result[ii]=num_roi;
		find++;
	    }
        }
...
template NIFTYSEG_WINEXPORT void ConsecutivelyConnectComp<unsigned char, unsigned char>(void * Old, void * New, ImageSize * Currentsize,bool spheric);
template NIFTYSEG_WINEXPORT void ConsecutivelyConnectComp<float, float>(void * Old, void * New, ImageSize * Currentsize,bool spheric);
