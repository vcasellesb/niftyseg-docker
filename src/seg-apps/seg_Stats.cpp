...
//    printf("\t-snp <ax>\t\t| Sum of all fuzzy voxels (sum(<in>))\n");
    ...
    printf("\t-map <in2> <csv>| Give the matching between the numbers in two different masks in <in> and <in2>. Save to <csv> file\n");
#ifdef _GIT_HASH
...
int main(int argc, char **argv)
{
    try
    {
		...
		filenames[1] = argv[++i];
                ...
                for(unsigned int index=0; index<Images[0]->nvox; index++)
                {
		    if(CountINTERSECT[(int)(Img1prt[index])]==0) {
			CountINTERSECT[(int)(Img1prt[index])]=(int)(Img2prt[index]);
		    }
		    else {
			if(CountINTERSECT[(int)(Img1prt[index])]!=(int)(Img2prt[index]) &&
				(int)(Img2prt[index])!=0) {
				CountINTERSECT[(int)(Img1prt[index])]=-1;
			}
		    }
                    maxclass=(int)(Img1prt[index])>maxclass?(int)(Img1prt[index]):maxclass;
                }
                ...
                myfile.open(argv[++i]);
                flush(cout);
		// We start in 1 to avoid background
                for(int curtclass=1; curtclass<=maxclass; curtclass++)
                {
                    myfile<<curtclass<<","<<CountINTERSECT[curtclass]<<endl;
                    cout<< "Label["<<curtclass<<"] = "<< CountINTERSECT[curtclass] <<endl;
                }
                myfile.close();
            }
            // **************************            ---------          *****************************
            ...
            else if((strcmp(argv[i], "-vp") == 0 || strcmp(argv[i], "-V") == 0) && (i)<argc)
            {
                if(Images[0]->datatype!=NIFTI_TYPE_FLOAT32)
                {
                    seg_changeDatatype<float>(Images[0]);
            ...
            else if(strcmp(argv[i], "-vl") == 0 && (i)<argc)
            {
                ...
                float * Img1prt = static_cast<float *>(Images[0]->data);
                int  CountIMG1[20000]= {0};
                int maxclass=0;
                int curlab=0;
                for(unsigned int index=0; index<Images[0]->nvox; index++)
                {
                    if((round(Img1prt[index]))>20000  || (round(Img1prt[index]))<0)
                    {
                        cout<<"Too many labels... the code only handles up to 20000 labels"<<endl;
                        seg_exit();
                    }
                    else
                    {
            ...
            else if((strcmp(argv[i], "-np") == 0 || strcmp(argv[i], "-Np")==0 || strcmp(argv[i], "-N")==0) && (i)<argc)
            {
                if(Images[0]->datatype!=NIFTI_TYPE_FLOAT32)
                {
                    seg_changeDatatype<float>(Images[0]);
