...
void Usage(char *exec)
{
    ...
    printf("\t-concomp6\t\tLabel the different connected components with a 6NN kernel (3D method)\n");
    printf("\t-concomp26\t\tLabel the different connected components with a 26NN kernel (3D method)\n");
    printf("\t-lccc6\t\t\tLabel consecutivelly the different connected components with a 6NN kernel (4D method)\n");
    printf("\t-lccc26\t\t\tLabel consecutivelly the different connected components with a 26NN kernel (4D method)\n");
    printf("\t-fill\t\t\tFill holes in binary object (e.g. fill ventricle in brain mask).\n");
    printf("\t-euc\t\t\tEuclidean distance transform\n");
...
//            }
            ...
            else if(strcmp(argv[i], "-euc") == 0)
            {
                ...
                for(long i=0; i<(long)(CurrSize->xsize*CurrSize->ysize*CurrSize->zsize*CurrSize->tsize*CurrSize->usize); i++)
                {
                    Lable[i]=bufferImages[current_buffer][i]>1;
                    Speed[i]=1.0f;
                }
            ...
            // *********************  Consecutively labeled Connected Components 6NN  *************************
            else if(strcmp(argv[i], "-lccc6") == 0)
            {
                ConsecutivelyConnectComp<float,float>(static_cast<void*>(bufferImages[current_buffer]),static_cast<void*>(bufferImages[current_buffer?0:1]),CurrSize,false);
                current_buffer=current_buffer?0:1;
            }
            // *********************  Consecutively labeled Connected Components 26NN  *************************
            else if(strcmp(argv[i], "-lccc26") == 0)
            {
                ConsecutivelyConnectComp<float,float>(static_cast<void*>(bufferImages[current_buffer]),static_cast<void*>(bufferImages[current_buffer?0:1]),CurrSize,true);
                current_buffer=current_buffer?0:1;
            }
